        ...
        <PreviewBox type={type} isShown={imagesBox} exit={() => showImages(false)}>
            {allImages.map(image =>
                <div key={image.id} onClick={() => addImage(image)} className='previewnote'>
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" className={images.indexOf(image)!==-1?'previewcheckedsvglink':'displaynone'} fill="none" viewBox="0 0 24 24" stroke="var(--color5)" strokeWidth={3}>
